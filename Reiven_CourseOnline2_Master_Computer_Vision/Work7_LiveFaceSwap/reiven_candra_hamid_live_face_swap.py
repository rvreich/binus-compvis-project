...
def annotate_landmarks(im, landmarks):
    ...
    for idx, point in enumerate(landmarks):
        ...
        cv.putText(im, str(idx), pos, fontFace=cv.FONT_HERSHEY_SCRIPT_SIMPLEX, fontScale=0.4, color=(0, 0, 255))
...
def transformation_from_points(points1, points2):
	...
	# the formulation assumes the matrix goes on the right(with row vectors)
...
def read_im_and_landmarks(fname):
    ...
    im = cv.resize(im,None,fx = 0.35, fy = 0.35, interpolation = cv.INTER_LINEAR)
...
def warp_im(im, M, dshape):
    ...
    cv.warpAffine(im, M[:2], (dshape[1], dshape[0]), dst=output_im, borderMode=cv.BORDER_TRANSPARENT, flags=cv.WARP_INVERSE_MAP)
    return output_im
...
def face_swap(img,name):
    ...
    frame = cv.resize(image,None,fx=1.5, fy=1.5, interpolation = cv.INTER_LINEAR)
...
while True:   
    ...
    frame = cv.resize(frame, None, fx=0.75, fy=0.75, interpolation = cv.INTER_LINEAR)
    
    # flip image so that it's more mirror like
    frame = cv.flip(frame, 1)
    
    cv.imshow('Reiven Face Swapper', face_swap(frame, filter_image))
